...
def generate_fixed_cid(len, ctr):
    return bytes([ord("A")+ctr]) * len
...
class QuicConnection:
    ...
    def __init__(
        ...
        *,
        ...
        tmp_cid = os.urandom(configuration.connection_id_length)
        self._host_cids = [
            QuicConnectionId(
                cid=hashlib.shake_256(tmp_cid).digest(configuration.connection_id_length),
                original_cid=tmp_cid,
                sequence_number=0,
    ...
    def _replenish_connection_ids(self) -> None:
        ...
        while len(self._host_cids) < min(8, self._remote_active_connection_id_limit):
            tmp_cid = os.urandom(self.configuration.connection_id_length)
            self._host_cids.append(
                QuicConnectionId(
                    cid=hashlib.shake_256(tmp_cid).digest(self.configuration.connection_id_length),
                    original_cid=tmp_cid,
